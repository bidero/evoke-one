...
// =========================================================================
// AJAX: zapisz CAŁY formularz ustawień, bez przeładowania strony
// =========================================================================

/**
 * OPCJE, KTÓRE WOLNO ZAPISAĆ CAŁYM FORMULARZEM.
 *
 * Osobna lista od `evk_toggle_allowlist()`, bo chroni przed czym innym. Tamta
 * przepuszcza WYŁĄCZNIE wartości 0/1 na wskazanym polu, więc nawet pomyłka
 * w nazwie opcji niewiele daje. Tutaj przez uchwyt przechodzi CAŁA TABLICA
 * ustawień, więc lista musi być krótka i czytana świadomie.
 *
 * Na razie jedna pozycja: zapis AJAX-em jest próbą kształtu na jednym ekranie.
 */
function evk_settings_allowlist(): array {
    return ['evk_sierotki'];
}
 ...
 *
 * SANITACJA IDZIE PRZEZ `update_option()`, nie przez wołanie filtra z ręki.
 * WordPress podpina `sanitize_callback` z `register_setting()` pod filtr
 * `sanitize_option_{$option}`, a `update_option()` tamtędy przechodzi. To nie
 * jest założenie na słowo: TĄ SAMĄ DROGĄ jedzie przełącznik AJAX od kilkudziesięciu
 * wydań, a `evk_preserve_toggle()` niżej istnieje wyłącznie dlatego, że callback
 * NAPRAWDĘ się wtedy odpala — gdyby nie odpalał, nie miałby czego zachowywać.
 *
 * Pierwsza wersja wołała filtr dodatkowo, jawnie, „na wszelki wypadek".
 * Zmierzone mutacją: usunięcie tego wołania nie zmieniało niczego, bo wartość
 * i tak przechodziła przez sito w `update_option()`. Kod, którego usunięcie
 * niczego nie psuje, nie broni przed niczym — poszedł.
 *
 * WARUNKIEM tej bezpiecznej drogi jest to, że opcja Z LISTY MA zarejestrowane
 * sito. Pilnuje tego osobne sprawdzenie: wpis bez `sanitize_callback` zapisywałby
 * surową tablicę z żądania prosto do bazy.
 *
 * Pola przychodzą jako SUROWY ŁAŃCUCH `$form.serialize()` i rozbieramy je
 * `parse_str()`, czyli tym samym, czym rozebrałby je PHP przy zwykłym POST.
 * Składanie tablicy z `name="evk_x[y][z]"` po stronie JS byłoby drugą, własną
 * implementacją tego parsera — i pierwszym miejscem, gdzie oba wyniki
 * rozjechałyby się przy zagnieżdżonym polu.
 */
add_action('wp_ajax_evk_save_settings', function () {
    check_ajax_referer('evk-save-settings', 'nonce');
    if (!current_user_can('manage_options')) wp_send_json_error('forbidden', 403);

    $option = sanitize_key($_POST['option'] ?? '');
    if (!in_array($option, evk_settings_allowlist(), true)) {
        wp_send_json_error('not_allowed: ' . $option, 403);
    }

    $pola = [];
    parse_str((string) wp_unslash($_POST['form'] ?? ''), $pola);

    /* Formularz bez własnej tablicy to nie jest „zapisz pustkę" — to znaczy, że
       przyszło coś innego, niż ten ekran wysyła. Zapisanie wtedy pustej tablicy
       skasowałoby ustawienia modułu. */
    if (!isset($pola[$option]) || !is_array($pola[$option])) {
        wp_send_json_error('brak danych dla ' . $option, 400);
    }

    update_option($option, $pola[$option]);

    wp_send_json_success(['option' => $option, 'wartosc' => get_option($option, [])]);
});
