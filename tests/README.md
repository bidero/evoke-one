...
| `anim-preview.test.js` | podgląd, który pokazuje co innego niż strona, jest gorszy niż jego brak — dlatego varsy podglądu porównywane są z tablicą presetów z PHP, a oba parsery `opacity: 0` (PHP i JS) muszą dawać ten sam wynik; SplitText przebudowuje DOM i wtyczki tekstowe nadpisują `innerHTML`, więc scena musi być przywracana przez SILNIK, nie przez wołającego (1.47.0); ▶ ma stać PRZY KADRZE — w pasku nagłówka przy rozwiniętym akordeonie widać było albo przycisk, albo pudełko, a poprzednie sprawdzenie pilnowało właśnie tamtego miejsca (1.49.0) |
...
| `__clipped` (w `admin-tabs`) | wystawanie poza okno to tylko połowa — kontener z `overflow-x: hidden` CHOWA to, co się w nim nie mieści, więc przycisk przestaje istnieć dla klikającego, a pomiar wystawania nadal świeci na zielono; tak zginęło „Wyczyść logi" w raportach newslettera przy oknie 360 px (1.49.0) |
| `dup-class` (w `admin-tabs`) | przeglądarka bierze PIERWSZY atrybut `class` i po cichu ignoruje resztę — klasa dopisana jako drugi nie działa, choć w źródle wygląda, jakby działała; przemiecenie zakładek wprowadziło tak 37 miejsc, w tym `is-ok`/`is-err` na ramkach informacyjnych, wydane w 1.44.0 i 1.45.0 (1.48.0) |
...
| `offcanvas.test.js` (krzywe i FOUC) | lista krzywych jest wspólna z Animatorem, więc niesie zapis GSAP-a — a nieznana funkcja czasu unieważnia CAŁĄ deklarację `transition`, razem z czasem trwania, więc wybranie krzywej nie zmieniało ruchu, tylko GASIŁO go do zera; korzeń z `display: contents` pokazuje panele w treści strony, dopóki JS ich nie przeniesie, i to widać jako mignięcie przy ładowaniu (1.61.0, 1.62.0) |
...
| `og-layers.test.js` | inicjalizacja przeciągania stała w `<script>` W TREŚCI zakładki, a `sortablejs` jedzie ze STOPKI — skrypt ruszał, zanim biblioteka trafiła na stronę, i cichy `if (typeof Sortable !== 'undefined')` połykał to bez śladu; ta sama przyczyna co w 1.37.1, więc fixture odtwarza KOLEJNOŚĆ skryptów wp-admin, a jedno sprawdzenie pilnuje, żeby zakładka w ogóle nie niosła własnego skryptu (1.60.0) |
